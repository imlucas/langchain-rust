...
//! # Examples
//!
//! `no_run //! use langchain_rust::tools::{Tool, WikipediaQuery}; //! use serde_json::json; //! //! #[tokio::main] //! async fn main() { //!     let wiki = WikipediaQuery::default(); //!     let result = wiki.run(json!("Rust programming language")).await.unwrap(); //!     println!("{}", result); //! } //! `
...
/// # Examples
///
/// `no_run /// use langchain_rust::tools::{Tool, WikipediaQuery, WikipediaQueryOptions}; /// use serde_json::json; /// /// #[tokio::main] /// async fn main() { ///     let options = WikipediaQueryOptions { ///         top_k_results: 2, ///         max_doc_content_length: 2000, ///         lang: "en".to_string(), ///     }; ///      ///     let wiki = WikipediaQuery::new(options); ///     let result = wiki.run(json!("LangChain")).await.unwrap(); ///     println!("{}", result); /// } /// `
